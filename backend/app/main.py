...
def _mcp_format_duration_minutes(seconds: Any) -> float:
    return round(max(_safe_float(seconds), 0.0) / 60.0, 1)


def _mcp_coerce_day_utc(value: str | None, *, default: Optional[date] = None) -> str:
    raw = str(value or "").strip()
    if not raw:
        if default is None:
...
def _lt_pace_for_day(db_path: Path, day_utc: str) -> float:
    ...
    return float(_curve_value_at(pace_curve, pace_default, day_ts))
...
def _lthr_for_day(db_path: Path, day_utc: str) -> float:
    ...
    if pd.isna(day_ts):
        raise ValueError(f"Invalid target_day_utc: {day_utc}")
    return float(_curve_value_at(lthr_curve, lthr_default, day_ts))


def _weekly_baseline_tss_for_day(db_path: Path, day_utc: str, owner: str = "") -> float:
...
def _mcp_preview_cycle_payload(
    *,
    owner: str,
    target_day_utc: str,
    ...
    day_utc = _mcp_coerce_day_utc(target_day_utc)
    db_path = _db_path_for_owner(owner)
    planning_state = _generated_activity_planning_state(
        db_path=db_path,
        day_utc=day_utc,
        threshold_pace_sec_per_km=_lt_pace_for_day(db_path, day_utc),
        methodology_id=methodology_id,
        schedule_constraints=schedule_constraints or [],
    )
    if planning_state is None:
    ...
    return {
        ...
        "preview": _mcp_preview_intents_payload(horizon),
        "recent_long_runs": [
            {
...
def _mcp_planning_context_payload(
    *,
    owner: str,
    target_day_utc: str,
    active_build: Optional[dict[str, Any]] = None,
    doctrine_resource_refs: Optional[list[str]] = None,
    today_status: Optional[dict[str, Any]] = None,
    methodology_id: Optional[str] = None,
    seed: Optional[int] = None,
    horizon_days: Optional[int] = None,
    schedule_constraints: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    normalized_day = _mcp_coerce_day_utc(target_day_utc)
    preview_payload = _mcp_preview_cycle_payload(
        owner=owner,
    ...
    planning_state = _generated_activity_planning_state(
        ...
        threshold_pace_sec_per_km=_lt_pace_for_day(db_path, normalized_day),
        methodology_id=methodology_id,
        schedule_constraints=schedule_constraints or [],
    )
    if planning_state is None:
    ...
    return {
        ...
        "target_day_utc": normalized_day,
        "db_path": str(db_path),
        "active_build": active_build or {},
        "planning_state_summary": _mcp_planning_state_summary(planning_state),
        "today_status": today_status or {},
        "preview_horizon": preview_payload["preview"],
        "preview_meta": preview_payload["preview_meta"],
        "recent_long_run_summary": preview_payload["recent_long_runs"],
        ...
        "doctrine_resource_refs": doctrine_resource_refs or [],
        "methodology_id": preview_payload["methodology_id"],
    }
...
def _mcp_modality_mix_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    duration_by_modality: Counter[str] = Counter()
    total_duration = 0.0
    for row in rows:
        duration_min = _mcp_format_duration_minutes(row.get("duration_s"))
...
def _mcp_history_snapshot_components(
    *,
    owner: str,
    window_days: int,
    ...
    planning_state = _generated_activity_planning_state(
        db_path=db_path,
        day_utc=reference_day_utc,
        threshold_pace_sec_per_km=_lt_pace_for_day(db_path, reference_day_utc),
        methodology_id=methodology_id,
        schedule_constraints=[],
    )
    if planning_state is None:
    ...
    return {
        ...
        "planned_rows": _aggregate_planned_days_for_planning(
            db_path=db_path,
            start_day=pd.Timestamp(start_day_utc),
            end_day=pd.Timestamp(normalized_end_day_utc),
        ),
    }


def _mcp_build_history_snapshot_payload(
    *,
    owner: str,
    window_days: int,
    end_day_utc: Optional[str] = None,
    ...
    snapshot = _mcp_history_snapshot_components(
        owner=owner,
    ...
    return {
        ...
        "load_summary": _mcp_load_summary(rows, snapshot["window_days"]),
        "hard_session_mix": _mcp_hard_session_mix(rows),
        "spacing_summary": _mcp_spacing_summary(rows),
        "long_run_summary": _mcp_long_run_summary(rows),
        "modality_mix": _mcp_modality_mix_summary(rows),
        "evidence_refs": evidence_refs or [],
    }


def _mcp_build_planned_comparison(snapshot: dict[str, Any]) -> dict[str, Any]:
    actual_rows = snapshot["actual_rows"]
    planned_rows = _mcp_classify_history_rows(
        snapshot["planned_rows"],
        snapshot["planning_state"].weekly_baseline_tss,
        snapshot["methodology"].stress_profile,
    )
    ...
    return {
        ...
        "actual_hard_days": _mcp_hard_session_mix(actual_rows).get("hard_day_count"),
        "planned_hard_days": _mcp_hard_session_mix(planned_rows).get("hard_day_count"),
    }


def _mcp_assess_history_against_doctrine(
    ...
    *,
    ...
    load_summary = _mcp_load_summary(actual_rows, snapshot["window_days"])
    hard_mix = _mcp_hard_session_mix(actual_rows)
...
def _mcp_build_history_judgment_payload(
    *,
    owner: str,
    window_days: int,
    end_day_utc: Optional[str] = None,
    ...
    structure_summary = {
        ...
        "spacing_summary": _mcp_spacing_summary(snapshot["actual_rows"]),
    ...
    payload = {
        ...
        "active_build": active_build_payload,
    ...
    if include_planned_comparison:
        payload["planned_comparison"] = _mcp_build_planned_comparison(snapshot)
    return payload
...
def _settings_view_core(db_path: Path) -> dict[str, Any]:
    from backend.app.settings_service import settings_view_core

    return settings_view_core(db_path)
...
def _settings_update_core(db_path: Path, settings: dict[str, Any]) -> dict[str, Any]:
    from backend.app.settings_service import settings_update_core

    return settings_update_core(db_path, settings)
