...
- Storage safeguards:
  ...
  - Garmin raw artifacts are role-scoped:
    - `admin`: raw archive + snapshots may persist
    - non-admin: temp ingestion only, then automatic cleanup (no persisted raw files/snapshots)
