    ...
    for chunk in chunks:
        ...
        if rep_match:
            ...
            if tss_input is not None and tss_input > 0 and bpm is None and pace is None and if_input is None:
                ...
                if seg_duration_h > 0:
                    if_input = max((per_rep_tss / 100.0) / seg_duration_h, 0.0) ** 0.5
                    if_input_source = "tss_derived"
                else:
                    if_input = None
                    if_input_source = None
            for rep_idx in range(reps):
                ...
                segments.append(seg)
                if recovery_chunk and (recovery_minutes or recovery_distance_km):
                    if recovery_tss_input is not None and recovery_tss_input > 0 and recovery_bpm is None and recovery_pace is None and recovery_if_input is None:
                        recovery_duration_h = (recovery_minutes / 60.0) if recovery_minutes else 0.0
                        if recovery_duration_h > 0:
                            recovery_if_input = max((float(recovery_tss_input) / 100.0) / recovery_duration_h, 0.0) ** 0.5
                            recovery_if_source = "tss_derived"
                        else:
                            recovery_if_input = None
                            recovery_if_source = None
                    rec: dict[str, float | str | None] = {
                        "kind": kind,
                        "minutes": recovery_minutes,
                        "distance_km": recovery_distance_km,
                        "bpm": recovery_bpm,
                        "pace_sec_per_km": recovery_pace,
                        "if_input": recovery_if_input,
                        "if_input_source": recovery_if_source,
                        "tss_input": recovery_tss_input,
                        "time_hint": line_time_hint,
                    }
                    segments.append(rec)
            last_kind = kind
        ...
        if repeated_distance is not None:
            reps, distance_km = repeated_distance
            if reps <= 0 or distance_km <= 0:
                warnings.append(f"Invalid repeated distance block in: `{chunk}`")
                continue
            if tss_input is not None and tss_input > 0 and bpm is None and pace is None and if_input is None:
                if pace is not None and threshold_pace_value > 0:
                    rep_minutes_est = distance_km * (pace / 60.0)
                elif threshold_pace_value > 0:
                    rep_minutes_est = distance_km * (threshold_pace_value / 60.0)
                else:
                    rep_minutes_est = None
                if rep_minutes_est and rep_minutes_est > 0:
                    seg_duration_h = rep_minutes_est / 60.0
                    per_rep_tss = float(tss_input) / float(max(reps, 1))
                    if_input = max((per_rep_tss / 100.0) / seg_duration_h, 0.0) ** 0.5
                    if_input_source = "tss_derived"
            for _ in range(reps):
        ...
        if minutes is None and distance_km is None:
            warnings.append(f"Missing duration or distance in: `{chunk}`")
            continue

        if tss_input is not None and tss_input > 0 and bpm is None and pace is None and if_input is None:
            seg_duration_h = (minutes / 60.0) if minutes else 0.0
            if seg_duration_h <= 0 and distance_km and threshold_pace_value > 0:
                seg_duration_h = (distance_km * threshold_pace_value) / 3600.0
            if seg_duration_h > 0:
                if_input = max((float(tss_input) / 100.0) / seg_duration_h, 0.0) ** 0.5
                if_input_source = "tss_derived"
